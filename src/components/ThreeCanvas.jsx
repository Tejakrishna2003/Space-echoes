...
export default function ThreeCanvas({ selectedBodyKey, onPlanetClick, timeSpeed, setFps, isCutawayOpen, hasStarted, setRenderStats, isConstellationsOpen }) {
  ...
  useEffect(() => {
    ...
    let cometTailGeo = null;
    let androPoints = null;
    ...
    constellationsGroup = buildConstellations(starTexture, trackResource);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 8. DEEP SPACE OBJECTS BUILDER
    // ─────────────────────────────────────────────────────────────────────────
    // A. Milky Way Galaxy
    const mw = buildMilkyWay(starTexture, trackResource, SPACE_DATA);
    const galaxyGroup = mw.galaxyGroup;
    sagittariusACoreMesh = mw.sagittariusACoreMesh;
    ...
    // B. Orion Nebula
    const neb = buildOrionNebula(starTexture, trackResource, SPACE_DATA);
    const nebulaGroup = neb.nebulaGroup;
    trapeziumStarsGroup = neb.trapeziumStarsGroup;
    ...
    nebulaTargetMesh.userData = { key: 'nebula', data: SPACE_DATA.nebula };
    scene.add(nebulaTargetMesh);
    planetMeshes.nebula = nebulaTargetMesh;
    interactiveObjects.push(nebulaTargetMesh);

    // C. Andromeda Galaxy
    const andro = buildAndromeda(starTexture, trackResource, SPACE_DATA);
    const androGroup = andro.androGroup;
    androPoints = andro.androPoints;
    ...
    androTargetMesh.userData = { key: 'andromeda', data: SPACE_DATA.andromeda };
    ...
    const bh = buildSupermassiveBlackHole(starTexture, trackResource, SPACE_DATA, getBlackHoleAccretionTexture);
    const bhGroup = bh.bhGroup;
    blackHoleAccretionRing = bh.blackHoleAccretionRing;
    jetGeo = bh.jetGeo;
    jetPositions = bh.jetPositions;
    jetSpeeds = bh.jetSpeeds;
    scene.add(bhGroup);
    ...
    bhTargetMesh.userData = { key: 'blackhole', data: SPACE_DATA.blackhole };
    scene.add(bhTargetMesh);
    planetMeshes.blackhole = bhTargetMesh;
    interactiveObjects.push(bhTargetMesh);

    // E. Comet
    const comet = buildComet(starTexture, trackResource);
    cometMesh = comet.cometMesh;
    cometComaMesh = comet.cometComaMesh;
    cometTailGeo = comet.cometTailGeo;
    const cometTailParticles = comet.cometTailParticles;
    const cometTailCount = comet.cometTailCount;
    scene.add(cometMesh);
    ...
    const handleTouchMove = (e) => {
      if (e.touches.length === 1 && isDragging) {
        ...
        targetZoomDistanceRef.current = THREE.MathUtils.clamp(
          targetZoomDistanceRef.current + delta * 0.15,
          2.0,
          250.0
        );
    ...
    const handleWheel = (e) => {
      ...
      targetZoomDistanceRef.current = THREE.MathUtils.clamp(
        targetZoomDistanceRef.current + normalizedDelta * 5.0,
        2.0,
        250.0
      );
    };
    ...
    const animate = () => {
      ...
      if (androPoints) androPoints.rotation.y += 0.006 * deltaTime;
      nebulaGroup.rotation.y += 0.004 * deltaTime;

      // Relativistic Plasma Jets Particle Animation
      if (jetGeo && jetPositions) {
        const positions = jetGeo.attributes.position.array;
        for (let i = 0; i < jetPositions.length; i++) {
          const p = jetPositions[i];
          const speed = jetSpeeds[i] * currentSpeed * deltaTime * 50;
          positions[i * 3 + 1] += speed * p.direction;

          if (Math.abs(positions[i * 3 + 1]) > 25.0) {
            positions[i * 3] = p.x;
            positions[i * 3 + 1] = 0;
            positions[i * 3 + 2] = p.z;
          } else {
            const currentHeight = Math.abs(positions[i * 3 + 1]);
            const scale = 1.0 + currentHeight * 0.12;
            const angle = Math.atan2(positions[i * 3 + 2], positions[i * 3]);
            positions[i * 3] = Math.cos(angle) * p.baseRadius * scale;
            positions[i * 3 + 2] = Math.sin(angle) * p.baseRadius * scale;
          }
        }
